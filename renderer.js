document.addEventListener("DOMContentLoaded", () => {
    ...
    function filterProducts(productsData, margin, buyVolumeThreshold, sellVolumeThreshold, buyPrice) {
    ...
    async function updateListView() {
        ...
        const filteredProducts = filterProducts(productsData, margin, buyVolumeThresholdValue, sellVolumeThresholdValue, buyPriceValue);

        productList.innerHTML = "";
        filteredProducts.forEach(product => {
            const li = document.createElement("li");
            li.textContent = `${product.productId}: Margin ${product.margin.toFixed(2)}`;

            const tooltip = document.createElement("div");
            tooltip.className = "tooltip";
            tooltip.innerHTML = `
                Buy Price: ${product.buyPrice.toFixed(2)}<br>
                Sell Price: ${product.sellPrice.toFixed(2)}<br>
                Margin: ${product.marginPercentage.toFixed(2)}%<br>
                Buy Volume: ${product.buyVolume}<br>
                Sell Volume: ${product.sellVolume}
            `;
